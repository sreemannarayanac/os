//program to implement Shortest Job First (SJF) scheduling algorithm without arrival time.
...
int main()
{
    ...
    for(i=0; i<n; i++) // sorting the processes according to their burst time
    {
        for(j=i+1; j<n; j++)
        {
            if(bt[i]>bt[j])
